...
import { Code } from "@styled-icons/ionicons-sharp/Code";
...
export const DownIcon = styled(FiChevronDown)`
  font-size: 40px;
  transform: ${({ open, backendOpen }) =>
    open || backendOpen || (open && backendOpen) ? "rotate(-180deg)" : null};
...
export const BracesIcon = styled(Code)`
  width: 50px;
  color: ${({ theme }) => theme.colors.basePurple};
  margin-right: 0.8rem;
`;
export const BackendIcon = styled(Data)`
  width: 50px;
  color: ${({ theme }) => theme.colors.basePurple};
  margin-right: 0.8rem;
`;
...
export const PercentageBar = styled.div`
  width: ${({ perc, backendPerc }) => perc || backendPerc}%;
...
export const BackendSkillsContainer = styled.div`
  margin: auto;
  width: 100%;
